...
class RoomTypes(models.Model):
    ...
    # Add a One2many field for charge history
    charge_history_ids = fields.One2many('hotel.roomtype.charge.history', 'roomtype_id', 
                                        string='Charge History')
    
    # Optional: Create a computed field to show current charge from history
    current_historical_charge = fields.Float(string='Current Historical Charge', 
                                           compute='_compute_current_charge', store=True)
    
    roomtypes_list = fields.Many2many(
        'hotel.roomtypes',
        compute='_compute_roomtypes_list',
        string='All Room Types',
        store=False
    )
...
class RoomTypeChargeHistory(models.Model):
    ...
    def _compute_is_current(self):
        today = date.today()
        for record in self:
            record.is_current = (record.date_from <= today and 
                              (not record.date_to or record.date_to >= today))